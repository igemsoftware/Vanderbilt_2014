...
#endif
    free(
      output_file_name); // this is absolutely tiny but there's no reason not to
                         // free it
...
result_bytes_processed_pair de_vcsfmt(char * filename) {
    ...
    string_with_size * input_block_with_size =
      make_new_string_with_size(BINBLOCK_SIZE);
    // both are BINBLOCK_SIZE because BINBLOCK_SIZE was calculated to be
    // efficient
    // at file I/O (at 8192 as I write this)
    // one might think BLOCK_SIZE should be used here, but that would only work
    // if,
    // as in the worst-case scenario,
    // two newlines are inserted for an ORF for every 60 bases, and would
    // produce a
    // segfault if not
    // since output is always smaller than input (due to removal of newlines),
    // no
    // segfault occurs
    string_with_size * output_block_with_size =
      make_new_string_with_size(BINBLOCK_SIZE);
