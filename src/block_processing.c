...
extern inline string_with_size *
  delimit_block_by_line(string_with_size * input_block_with_size_,
...
void read_and_process_block_vcsfmt_CONCURRENT(
 ...
 while (!feof(args->input_file) && !ferror(args->input_file)) {
  add_to_bytes_processed(
    ...
    read_block(args->input_file, args->input_block_with_size)->readable_bytes);
  // OPTIMIZATION: allocate from (possibly self-growing) pool of memory instead
  // of constantly heap allocating a completely new block
  // if doing so, use gasyncqueue only to pass indices into this pool, instead
  // of the actual memory blocks themselves
...
extern inline bool is_processing_complete_vcsfmt_CONCURRENT(
  read_write_block_args_CONCURRENT * args);
#endif

extern inline unsigned long djb2_hash_on_string_index(
