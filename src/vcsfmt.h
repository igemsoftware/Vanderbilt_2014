...
result_bytes_processed_pair vcsfmt(
  char * filename); // if filetype supported, produces vcsfmt file of same name
// MUST call free_result_bytes_processed_pair_not on the return value! allocated
...
result_bytes_processed_pair
  de_vcsfmt(char * filename); // produces original file
// MUST call free_result_bytes_processed_pair_not on the return value! allocated
// memory should be freed!

// remove newlines, play with other metadata before going into dna; produces
// file in .dwn folder
// returns indices of of bytes in file where dna exists, ensures DNA is
// divisible by 3
