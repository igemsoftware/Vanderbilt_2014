...
extern inline bool string_id_equal (string_id * a,
																		string_id * b);

void set_bool_if_string_id_match (string_id * prev_string_id,
																	boolean_and_data * bool_data_bundle){
	if (string_id_equal(prev_string_id,
											bool_data_bundle->data)){
...
#error FUNCTIONALITY NOT IMPLEMENTED YET
	...
	// // written inline instead of in separate function because it is very small and simple
	// string_id prev_string_ids[QUEUE_HASH_CRITICAL_SIZE];
	// string_id cur_string_ids[QUEUE_HASH_CRITICAL_SIZE];
	// size_t prev_front = 0;				// location of first element inserted
	// size_t prev_back = 0;	// one past final element, mod QUEUE_HASH_CRITICAL_SIZE, therefore 0
	// size_t prev_string_ids_filled = 0; // fill array up to QUEUE_HASH_CRITICAL_SIZE before moving start and end marks
	// size_t cur_front = 0;
	// size_t cur_back = 0;
	...
	// 	if (prev_string_ids_filled < QUEUE_HASH_CRITICAL_SIZE){
	// 		for (prev_string_index = 0;
	// 				 prev_string_index < BINBLOCK_SIZE;
	// 				 ++prev_string_index){
	// 			prev_instantaneous_hash = djb2_hash_on_string_index(prev_instantaneous_hash,
	// 																													)
	// 		}
	// 		// add one and just increment back
	// 	}
	// 	else{
	// 		// add one and move around queue markers if required
	// 	}
	// 	if (cur_string_ids_filled < QUEUE_HASH_CRITICAL_SIZE){
	// 		// add one and just increment back
	// 	}
	// 	else{
	// 		// add one and move around queue markers if required
	...
	while ((!(feof(prev_file) || ferror(prev_file)) || // until both files EOF
		...
		if (!(feof(prev_file) || ferror(prev_file)) &&
				g_queue_get_length(prev_file_string_ids_queue) < QUEUE_HASH_CRITICAL_SIZE){
			prev_block = read_block(prev_file, // assignment not necessary, but clearer
															prev_block);
			// OPTIMIZATION: move this index outside of the for loop
			for (size_t prev_block_index = 0;
					 prev_block_index < BINBLOCK_SIZE;
					 ++prev_block_index){
				...
				else{								// can do just "else" because previously formatted at vcsfmt
					prev_file_instantaneous_hash = djb2_hash_on_string_index(prev_file_instantaneous_hash,
																																	 prev_block->string,
																																	 prev_block_index);
					++prev_file_instantaneous_length;
				}
		...
		if (!(feof(cur_file) || ferror(cur_file)) &&
				g_queue_get_length(cur_file_string_ids_queue) < QUEUE_HASH_CRITICAL_SIZE){
			cur_block = read_block(cur_file, // assignment not necessary, but clearer
														 cur_block);
			for (size_t cur_block_index = 0;
					 cur_block_index < BINBLOCK_SIZE;
					 ++cur_block_index){
				if (cur_block->string[cur_block_index] == '\n'){
					g_queue_push_tail(cur_file_string_ids_queue,
					...
					cur_file_instantaneous_hash = djb2_hash_on_string_index(cur_file_instantaneous_hash,
																																	cur_block->string,
																																	cur_block_index);
					++cur_file_instantaneous_length;
				}
...
#error FUNCTIONALITY NOT IMPLEMENTED YET
					...
					g_queue_pop_head(cur_file_string_ids_queue); // NOT popping prev queue
					mpz_add_ui(lines_processed,
										 lines_processed,
										 1);
				}
