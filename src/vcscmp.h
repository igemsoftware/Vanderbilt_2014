...
inline string_id * make_string_id (unsigned long str_hash,
	...
	return string_id_set_str_length(
		string_id_set_str_hash(
			malloc(sizeof(string_id)),
			str_hash),
		str_length);
}
inline bool string_id_equal (string_id * a,
														 string_id * b){
	return a->str_hash == b->str_hash && a->str_length == b->str_length;
}

// modify LINES_ABOVE_BELOW_TO_SEARCH, not QUEUE_HASH_CRITICAL_SIZE which relies on it
#define LINES_ABOVE_BELOW_TO_SEARCH 5	// lines to search above and below active lines for same hashes
#define QUEUE_HASH_CRITICAL_SIZE 2*LINES_ABOVE_BELOW_TO_SEARCH + 1 // size of line block

// not inlined since is_string_id_in_prev_file takes its address, which disallows inlining
...
inline bool is_string_id_in_prev_queue (GQueue * prev_file_queue,
	...
	g_queue_foreach(prev_file_queue,
									(GFunc) set_bool_if_string_id_match,
									&bool_data_bundle);
