...
if_close_levenshtein_dist_add_to_list(line_id *prev_line_id,
                                      line_id_with_edit_match_info *cur_data) {
...
//     PRINT_ERROR_NEWLINE();
// #endif
    size_t leven_dist = get_levenshtein_distance(prev_line_id->first_k_chars,
                                                 cur_data->id->first_k_chars);
...
static inline boolean_and_data get_if_edit_line_and_if_so_add_to_list(
  ...
  g_queue_foreach(prev_file_queue, (GFunc)if_close_levenshtein_dist_add_to_list,
                  &liam);
...
static inline void write_line_and_if_new_add_to_list(
    ...
    mpz_t *cur_file_lines_processed, mpz_t *output_file_lines_processed,
    bool *break_out_of_vcscmp, GSList **edit_matches,
    FILE *prev_file_used_for_edits, FILE *cur_file, FILE *out_file) {
#ifdef DEBUG
...
static inline void if_within_first_section_write_to_string(
    ...
    string_with_size *sws_block, size_t ptr_index) {
...
static inline void react_to_next_character_of_block(
    string_with_size *input_block, size_t block_index, mpz_t *lines_processed,
    bool *is_line_orf, string_with_size **first_few_chars,
    bool *ptr_past_k_chars, unsigned long int *ptr_line_length,
    GQueue *ids_queue, unsigned long int *ptr_line_hash) {
...
static inline void add_blocks_to_queue(
    FILE *active_file, GQueue *ids_queue, string_with_size *input_block,
    bool *ptr_past_k_chars, unsigned long int *ptr_line_length,
    unsigned long int *ptr_line_hash, string_with_size **first_few_chars,
