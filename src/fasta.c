...
void fasta_de_vcsfmt(char * filename) {
	...
	getline(&(input_block_with_size->string), input_block_with_size->size_in_memory, input_file);

	// Parse the FASTA header.
	long line_alignment, metadata_size;
	fasta_parse_header(input_block_with_size->string, &line_alignment, &metadata_size);
    ...
    while (bytes_read < metadata_location &&
    		!ferror(input_file) && !ferror(output_file) && !ferror(metadata_file)) {
    	// Read a block of genetic data
        read_block(input_file, input_block_with_size);

        bytes_read += input_block_with_size->readable_bytes;

        // If we're near the end, only pay attention to the bytes before the start of the
        // metadata.
...
void fasta_parse_header(char * header, long * line_alignment, long * metadata_size) {
	// "VCSFMT;FASTA;" is 13 characters long. The first number after that is our line alignment.
