...
void fasta_vcsfmt(char * filename) {
  ...
  *is_processing_complete = false;
  ...
  while (!feof(temporary_file) && !ferror(temporary_file)) {
    write_block(output_file, read_block(temporary_file, input_block_with_size));
  }
...
void fasta_de_vcsfmt(char * filename) {
  ...
  // Read the FASTA header into the input block.
  getline(&(input_block_with_size->string),
          input_block_with_size->size_in_memory, input_file);

  // Parse the FASTA header.
  long line_alignment, metadata_size;
  fasta_parse_header(input_block_with_size->string, &line_alignment,
                     &metadata_size);

  // Open another file descriptor pointing to the beginning of the metadata
  FILE * metadata_file = open_file_read(filename);
  fseek(metadata_file, -metadata_size, SEEK_END);

  // Remember how many bytes it takes to get to the metadata
  long metadata_location = ftell(metadata_file);

  // Keep track of how many bytes we've parsed and lines we've written.
  ...
  while (bytes_read < metadata_location && !ferror(input_file) &&
         !ferror(output_file) && !ferror(metadata_file)) {
    // Read a block of genetic data
    read_block(input_file, input_block_with_size);

    bytes_read += input_block_with_size->readable_bytes;

    // If we're near the end, only pay attention to the bytes before the start
    // of the
    // metadata.
    if (bytes_read > metadata_location)
      input_block_with_size->readable_bytes -= bytes_read - metadata_location;

    // Remove all the new lines
    de_process_block_vcsfmt(input_block_with_size, output_block_with_size);

    // If we need to read more metadata into our block, do it.
    if (!feof(metadata_file) &&
        metadata_block_pos > metadata_block_with_size->readable_bytes) {
      read_block(metadata_file, metadata_block_with_size);
...
void fasta_parse_header(char * header,
                        long * line_alignment,
                        long * metadata_size) {
  // "VCSFMT;FASTA;" is 13 characters long. The first number after that is our
  // line alignment.
  *line_alignment = atol(header + 13);

  // Find the first ';' after the initial portion.
  char * next = strchr(header + 13, ';');

  // The number after that is the metadata size.
  *metadata_size = atol(++next);
}
